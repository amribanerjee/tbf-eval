...
True Authors: Carlos E. Jimenez, John Yang, Alexander Wettig, Shunyu Yao, Kexin Pei, Ofir Press, Karthik Narasimhan (Princeton NLP).

Technical Reality: Introduces a testbed of 2,294 engineering problems from 12 open-source Python repositories (e.g., scikit-learn, matplotlib, sympy). Evaluation relies entirely on applying an LLM-generated patch file to a repository environment and executing its unit test suite to yield a binary $1$ (Resolved) or $0$ (Unresolved).

Citation Justification: This serves as the primary evaluation standard and source dataset for your agent trajectories. You cite it to anchor the current paradigm of automated, execution-based evaluation, paving the way for your argument that while outcome validation is objective, it hides structural flaws in the trajectory itself.
...
Citation Justification: You will use their publicly released SWE-agent execution logs as part of your primary dataset. You cite it in Section 2 and 4 to illustrate how structural variations in an ACI shape the resulting step trajectories, providing the raw material that Trajectory Behavioral Fingerprinting (TBF) converts into behavioral attributions.
...
Citation Justification: This is the core validation for your Introduction (Section 1) and Discussion (Section 6). You cite it to directly justify the critical need for process metrology over outcome metrics, anchoring your framework as a technical instrument to bridge the "measurement gap" they expose. 
